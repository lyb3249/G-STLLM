...
class GraphModule(nn.Module):
    ...
    def forward(self, node_input):  # node_input: [B, N, F]
        ...
        for b in range(node_input.size(0)):
            h = self.gnn(node_input[b], self.edge_index)  # [N, D]
...
#
#         x_enc = self.normalize_layers(x_enc, 'norm') #Normalize(configs.enc_in, affine = False)
#
#         B, T, N = x_enc.size()
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        static = self.node_coordsunsqueeze(0).expand(B, -1, -1).to(x_enc.device)  # [B, N, 2]
