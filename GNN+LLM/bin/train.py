...
# 데이터 준비
train_loader, target_idx, node_coords = data_provider(args, flag='train')
vali_loader, _, _ = data_provider(args, flag='val')
test_loader, _, _ = data_provider(args, flag='test')

args.target_idx = target_idx
args.node_coords = node_coords

model = Model(args).to(args.device)
criterion = nn.MSELoss()
optimizer = optim.Adam(model.parameters(), lr=args.learning_rate)
early_stopping = EarlyStopping(patience=5, verbose=True, path=f"./checkpoints/{args.task_name}_{args.llm_model}_{args.seq_len}_{args.pred_len}.pt")
...
for epoch in range(args.train_epochs):
    ...
    for batch_x, batch_y, batch_x_mark, batch_y_mark in train_loader:
        ...
        loss = criterion(outputs.squeeze(-1), batch_y[:, -args.pred_len:, target_idx])
        loss.backward()
        optimizer.step()
...
model.load_state_dict(torch.load(f"./checkpoints/{args.task_name}_{args.llm_model}_{args.seq_len}_{args.pred_len}.pt"))
model.eval()
preds, trues = [], []
with torch.no_grad():
